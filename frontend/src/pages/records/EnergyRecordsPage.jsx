...
function EnergyRecordsPage() {
    ...
    const [formData, setFormData] = useState({
        departmentId: '', consumptionKwh: '',
        energyType: 'ELECTRICITY', recordedDate: '', notes: ''
    });
    ...
    const loadRecords = async () => {
        try {
            ...
            setError('Failed to load energy records');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async () => {
        if (!formData.departmentId || !formData.consumptionKwh
                || !formData.recordedDate) {
            setFormError(
                'Department ID, consumption, and date are required');
            return;
        }
        setSaving(true);
        try {
            if (selectedRecord) {
                await energyRecordsApi.update(
            ...
            <Dialog open={dialogOpen}
                ...
                <DialogContent>
                    ...
                    <TextField
                        ...
                        sx={{ mt: 1, mb: 2 }} />
                    <TextField
                        fullWidth label="Consumption (kWh)"
                        name="consumptionKwh" type="number"
