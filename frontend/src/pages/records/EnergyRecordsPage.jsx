...
import {
  ...
  Tooltip, Typography, Select, MenuItem, FormControl,
  InputLabel,
} from '@mui/material';
...
export default function EnergyRecordsPage() {
  ...
  const [form, setForm]                 = useState({ departmentId: '', consumptionKwh: '', energyType: 'ELECTRICITY', recordedDate: '', notes: '' });
  const [formErr, setFormErr]           = useState('');
  const [saving, setSaving]             = useState(false);
  const [delId, setDelId]               = useState(null);
  const [snack, setSnack]               = useState({ open: false, message: '', severity: 'success' });

  // Determine org on mount
  useEffect(() => {
    const init = async () => {
      if (isAdmin()) {
  ...
  const loadRecords = async () => {
    try {
      setLoading(true);
      const res = await energyRecordsApi.getByOrganization(selectedOrgId);
      setRecords(res.data);
  ...
  const openCreate = () => {
    setSelected(null);
    setForm({ departmentId: '', consumptionKwh: '', energyType: 'ELECTRICITY', recordedDate: '', notes: '' });
    setFormErr('');
    setOpen(true);
  };
  ...
  const openEdit = rec => {
    setSelected(rec);
    setForm({ departmentId: rec.departmentId, consumptionKwh: rec.consumptionKwh, energyType: rec.energyType, recordedDate: rec.recordedDate, notes: rec.notes || '' });
    setFormErr('');
    setOpen(true);
  };
  ...
  const save = async () => {
    if (!form.departmentId || !form.consumptionKwh || !form.recordedDate) {
      setFormErr('Please select a department, enter consumption, and choose a date.');
      return;
    }
    setSaving(true);
    try {
      if (selected) await energyRecordsApi.update(selected.id, form);
      else          await energyRecordsApi.create(form);
      setOpen(false);
      loadRecords();
      setSnack({ open: true, message: selected ? 'Record updated.' : 'Record created.', severity: 'success' });
    } catch (err) { setFormErr(err.response?.data?.error || 'Failed to save record.'); }
    finally { setSaving(false); }
  };

  const del = async id => {
    try {
      await energyRecordsApi.delete(id);
      setDelId(null);
      loadRecords();
      setSnack({ open: true, message: 'Record deleted.', severity: 'success' });
    } catch (err) {
      setDelId(null);
      setSnack({ open: true, message: err.response?.data?.error || 'Cannot delete record.', severity: 'error' });
    }
  };
  ...
  const columns = [
    ...
    { key: 'energyType', label: 'Type', render: v => <StatusBadge value={v} showDot /> },
    { key: 'consumptionKwh', label: 'Consumption', render: v => <Box sx={{ fontSize: 14, fontWeight: 600 }}>{Number(v).toLocaleString()} <Box component="span" sx={{ fontSize: 12, color: tokens.colors.textSec }}>kWh</Box></Box> },
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <PageShell title="Energy Records" subtitle="Electricity, natural gas, and renewable energy consumption records." breadcrumbs={[{ label: 'Carbon Tracking' }]}
      ...
      <DataTable columns={columns} rows={records} searchKeys={['energyType', 'recordedDate']}
        emptyTitle="No energy records" emptyDescription="Start logging energy consumption to calculate CO₂ emissions." emptyEmoji="⚡"
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth aria-labelledby={DIALOG_ID}>
        <DialogTitle id={DIALOG_ID}>{selected ? 'Edit Energy Record' : 'New Energy Record'}</DialogTitle>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            {!selected && (
              <DepartmentSelector
                ...
                onChange={deptId => setForm(p => ({ ...p, departmentId: deptId }))}
            ...
            <Box>
              <Box sx={{ fontSize: 13, fontWeight: 600, color: tokens.colors.text, mb: 0.75 }}>Consumption (kWh) *</Box>
              <TextField fullWidth name="consumptionKwh" type="number" inputProps={{ step: '0.01' }} value={form.consumptionKwh} onChange={handle} />
            </Box>
            <Box>
              <Box sx={{ fontSize: 13, fontWeight: 600, color: tokens.colors.text, mb: 0.75 }}>Energy Type</Box>
              <FormControl fullWidth>
                <Select name="energyType" value={form.energyType} onChange={handle} size="small">
                  {ENERGY_TYPES.map(t => <MenuItem key={t} value={t} sx={{ fontSize: 14 }}>{t}</MenuItem>)}
                </Select>
              </FormControl>
            </Box>
            <Box>
