...
export default function ServerRecordsPage() {
  ...
  const [form, setForm]       = useState({ departmentId: '', usageHours: '', serverType: 'CLOUD', recordedDate: '', notes: '' });
  ...
  const loadRecords = async () => {
    try {
      setLoading(true);
      const res = await serverRecordsApi.getByOrganization(selectedOrgId);
      setRecords(res.data);
  ...
  const openCreate = () => { setSelected(null); setForm({ departmentId: '', usageHours: '', serverType: 'CLOUD', recordedDate: '', notes: '' }); setFormErr(''); setOpen(true); };
  const openEdit = rec => { setSelected(rec); setForm({ departmentId: rec.departmentId, usageHours: rec.usageHours, serverType: rec.serverType, recordedDate: rec.recordedDate, notes: rec.notes || '' }); setFormErr(''); setOpen(true); };
  const handle = e => setForm(p => ({ ...p, [e.target.name]: e.target.value }));

  const save = async () => {
    if (!form.departmentId || !form.usageHours || !form.recordedDate) { setFormErr('Please select a department, enter usage hours, and choose a date.'); return; }
    setSaving(true);
    try {
      if (selected) await serverRecordsApi.update(selected.id, form);
      else          await serverRecordsApi.create(form);
  ...
  const del = async id => {
    try { await serverRecordsApi.delete(id); setDelId(null); loadRecords(); setSnack({ open: true, message: 'Record deleted.', severity: 'success' }); }
  ...
  const columns = [
    ...
    { key: 'serverType', label: 'Type', render: v => <StatusBadge value={v} showDot /> },
    { key: 'usageHours', label: 'Usage', render: v => <Box sx={{ fontSize: 14, fontWeight: 600 }}>{Number(v).toLocaleString()} <Box component="span" sx={{ fontSize: 12, color: tokens.colors.textSec }}>hrs</Box></Box> },
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <PageShell title="Server Usage" subtitle="Data center and cloud infrastructure energy consumption." breadcrumbs={[{ label: 'Carbon Tracking' }]}
      ...
      <DataTable columns={columns} rows={records} searchKeys={['serverType', 'recordedDate']}
        emptyTitle="No server records" emptyDescription="Log server and cloud usage to track infrastructure emissions." emptyEmoji="🖥️"
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth aria-labelledby={DIALOG_ID}>
        <DialogTitle id={DIALOG_ID}>{selected ? 'Edit Server Record' : 'New Server Record'}</DialogTitle>
        <DialogContent sx={{ pt: 2, pb: 1 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            <Box>
              <Box sx={{ fontSize: 13, fontWeight: 600, color: tokens.colors.text, mb: 0.75 }}>Usage Hours *</Box>
              <TextField fullWidth name="usageHours" type="number" inputProps={{ step: '0.01' }} value={form.usageHours} onChange={handle} />
            </Box>
            <Box>
              <Box sx={{ fontSize: 13, fontWeight: 600, color: tokens.colors.text, mb: 0.75 }}>Server Type</Box>
              <FormControl fullWidth>
                <Select name="serverType" value={form.serverType} onChange={handle} size="small">
                  {SERVER_TYPES.map(t => <MenuItem key={t} value={t} sx={{ fontSize: 14 }}>{t}</MenuItem>)}
                </Select>
              </FormControl>
            </Box>
            <Box>
