...
public class TravelRecordService {
    ...
    public List<TravelRecordResponseDTO> getByOrganization(Long organizationId) {
        ...
        return deptIds.stream()
                .flatMap(deptId -> travelRecordRepository.findByDepartmentId(deptId).stream())
                .sorted(java.util.Comparator.comparing(TravelRecord::getRecordedDate).reversed())
                .map(this::mapToDTO)
                .toList();
    }
